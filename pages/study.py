...
def load_dicom_image(dicom_path: str) -> tuple[np.ndarray, float, float]:
    ...
    window_width: float =  dicom_array.max() - dicom_array.min() # dicom.WindowWidth if 'WindowWidth' in dicom else
    window_level: float =  (dicom_array.max() + dicom_array.min()) / 2 # dicom.WindowLevel if 'WindowLevel' in dicom else
...
def main() -> None:
    """
    Main application logic after login. Loads user configurations, manages file navigation,
    annotations, and saves results.
    """
    ...
    try:
        try:
            probe: int = df[~(df['annotated'])].ID.tolist()[0]
        except IndexError:
            done = True
    ...
    if not done:
        ...
        if run_mode == "debug":
            ...
            st.header(f'{descriptor_yml["case"]} {st.session_state.file_index + 1}/{len(files)} - {"☑" if is_annotated else "☐"}')
